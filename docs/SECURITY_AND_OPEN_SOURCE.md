...
| 1.6.0 | 2026-07-31 | Project Owner Decision | 将默认边界调整为最大合法复用；宽松许可证可在实现 PR 内直接采用，特殊许可证在履行实际义务时不作类别性禁止 |
...
默认政策为 `MAXIMUM_LAWFUL_REUSE`。对已核验的宽松许可证项目，开发任务
可以直接选择依赖、Fork、完整 Vendor、Submodule、选择性复制或修改，并在
同一实现 PR 中完成版本、来源、许可证、路径和修改登记，不要求额外的复用
许可审批。Copyleft、非商业和自定义许可证也不作类别性禁止；当前用途和
分发方式满足实际条款时，可以采用条款允许的最宽模式。RECA 不能豁免任何
上游许可证、归属、NOTICE、源码提供、相同许可证或非商业限制。
