...
- `Research`：汇总所有实际引入项目的版本、许可证、来源、限制、缓存和回退记录，不把仅研究项目列为依赖。
- `Spike`：在主演示数据上进行冷/热启动、资源、断网、服务失败、缓存失效和恢复彩排。
- `Decision`：冻结发布版本、离线/Recorded 策略、第三方降级、性能预算和延期能力；Critical 可达风险按发布门禁处理。
- `Integration`：完成锁定、缓存、Notices、许可证/归属、ReproPackage 清单和演示性能验收；失败/预处理结果必须显式标记。
