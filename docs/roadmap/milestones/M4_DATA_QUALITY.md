...
- `Research`：读取 Pandera、Great Expectations 和 DVC 研究记录及 ADR-004。
- `Spike`：验证 Pandera lazy FailureCase 转换、误报、规则版本和性能；比较 GX 报告设计价值与第二运行时成本；验证 DVC 仅用于 fixture/provenance 的必要性。
- `Decision`：Pandera 是 P0 运行时验证引擎；GX 仅作设计/测试参考；DVC 不替代 DatasetVersion，是否安装由真实 fixture 需求决定。
- `Integration`：正式质量结果归一为 DataQualityRun/DataQualityIssue，输入版本不变；引用上游测试或文案时保留来源。
