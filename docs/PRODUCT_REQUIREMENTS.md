...
RECA 0.1 当前面向个人使用和校级比赛，优先保证有效功能、演示稳定和可信
科研闭环，不建设企业安全管理后台、组织治理或大规模生产运维产品能力。
...
4. 影响正式科研事实、数据版本或不可逆结果的高风险步骤需要正式审批；只读、扫描、候选生成和预览可以自动执行，低风险采用使用轻量确认。
...
开源复用不是面向用户的产品功能，而是交付策略。许可证允许、来源和归属
明确时，团队可以采用成熟依赖、服务、Fork、Vendor、Submodule 或选择性
复制，以提升效果和交付速度；复用不得改变本文件的 Requirement ID、P0
范围、成功指标或科研真实性要求。
...
企业安全后台、法规请求和正式生产安全运营同样不属于 Competition Edition
产品范围；基础认证、项目隔离和高风险科研审批仍按安全与数据契约保留。
