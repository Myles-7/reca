...
实际引入的第三方能力只有在以下条件全部有证据时，才可进入对应里程碑或比赛发布：固定版本和来源；许可证/归属完整；主案例效果通过；失败和资源边界可见；Offline/Recorded 路径可复现；输出经 RECA Schema 转换；项目隔离通过；ReproPackage 或运行记录包含实现元数据。

以下状态不得混淆：研究报告完成不等于运行时引入，依赖存在不等于业务能力完成，M0 GROBID 健康检查不等于 PDF 解析闭环，TanStack Table 包存在不等于文献工作台完成。发布报告必须逐项标为 `PASS`、`FAIL`、`NOT RUN` 或 `NOT APPLICABLE`，并说明所用真实服务、Recorded fixture、缓存或降级路径。
...
* [ ] 第三方许可证已记录；
* [ ] 上游仓库与 Commit/Tag 已记录；
* [ ] Fork/Vendor/Submodule/选择性复制所需 LICENSE、NOTICE 和归属完整；
* [ ] 特殊许可证文件与根许可证声明隔离；
* [ ] 无许可证或来源不明内容未进入正式仓库；
* [ ] ARS-Codex 标记为 `NONCOMMERCIAL_INTENT_DECLARED`，并保留商业化或用途变化复审门；
* [ ] 未将没有实际复制的 ARS-Codex 内容登记为已使用；
* [ ] 数据许可证已记录；
* [ ] 演示材料无真实敏感信息；
* [ ] 无 API Key 提交；
* [ ] 无未授权论文分发；
...
13. 低风险只读与候选生成工具不被无差别要求正式审批，高风险数据、正式结果和文件变更仍有有效审批。
