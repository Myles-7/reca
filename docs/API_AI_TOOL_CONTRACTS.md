...
只读查询、检索、解析、质量扫描、候选生成和预览可在权限、项目隔离、Schema、审计和数据访问边界内自动执行，不要求正式审批。采用低风险候选结果使用 `LIGHT_CONFIRMATION`；修改科研数据、正式结果或文件使用 `FORMAL_APPROVAL` 并绑定有效 `ApprovalRecord`。这些名称是规范性审批模式，不在本阶段新增 API 字段、Schema 或稳定枚举；现有契约字段保持不变。
...
只读和候选生成 Tool 的 `approval_required` 为 false；低风险采用动作需要轻量确认；改变数据、正式结果或文件的 Tool 必须正式审批。禁止能力不进入白名单。轻量确认与正式审批的具体表达优先复用现有契约，不得为同步政策擅自增加请求字段。
...
5. 校验乐观锁、幂等键和审批前置条件；
6. 计算最小必要数据访问范围；
7. 通过业务 Service 调用选定的直接库、已登记 Adapter 或隔离服务/Vendor 边界；
