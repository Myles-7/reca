...
| PyAlex | `875c708cbb6e449feebc46d2a7a26af8ed8b2fdd` | MIT | OpenAlex search, filtering, cursor pagination and raw provenance | `DIRECT_DEPENDENCY_WITH_PROVIDER` | M2 | Existing `0.21` Provider boundary; SearchRun integration planned | RECA `QueryPlan`, search run and normalized `LiteratureRecord` candidates | RECA Recorded transport and offline fixture replay | Recorded/offline tests, bounded retry, DOI/author normalization and no PyAlex objects outside provider |
| GROBID | `c3229a4b9ba1e9eb8ec8327c2ec9eed1581d410c` | Apache-2.0 | Scholarly PDF header/full-text/reference TEI parsing | `INDEPENDENT_SERVICE` | M2 parsing; M3 evidence validation | M2 Adapter, raw TEI Artifact and RECA Converter implemented | Immutable PDF/TEI Artifacts and RECA converter objects | Explicit pypdf degraded parser path | Pinned image/digest, resource/error corpus, raw TEI retention, page validation and visible degradation |
| grobid-client-python | `161e0f45189c8592b2e2c58e9638cc6218bc75fb` | Apache-2.0 | GROBID transport, request options, bounded concurrency and `503` handling | `RESEARCH_REFERENCE` | M2 | Spike completed; package not installed | RECA Worker Job, Artifact and client interface | Selected small RECA-owned `httpx` client | Direct transport selected; no copied source, filesystem or batch ownership |
