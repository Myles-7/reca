...
| M1-ISSUE-0009 | M1-8 | HIGH | RESOLVED | Artifact package tracking | A root ignore rule excluded the Artifact implementation and focused tests from Git. | NO | NO |
...
- Severity: HIGH
- Status: RESOLVED
- Area: Artifact package tracking
- Summary: The repository-root `artifacts/` ignore rule also matched
  `backend/app/artifacts/` and `backend/tests/artifacts/`, so local validation
  used files that were absent from a clean Git checkout.
- Evidence: GitHub Actions run `30679139459` reported `I001` because
  `app.artifacts` was absent during import classification; Compose job
  `91312443895` then showed the API repeatedly exiting with
  `ModuleNotFoundError: No module named 'app.artifacts'`.
- Affected requirement/contract: M1 Artifact implementation and tests; remote
  backend-quality, compose-smoke, and e2e-smoke gates.
- Impact: Resolved; the M1 Artifact package and its focused validation are now
  part of the auditable repository baseline and container build context.
- Safe workaround / deferred behavior: No workaround remains.
- Blocks current subtask: NO.
- Blocks M1 Exit Gate: NO.
- Suggested repair: Scope the runtime-output ignore to the repository-root
  `/artifacts/` directory and commit the backend Artifact package and focused
  tests.
- Resolution evidence: `.gitignore` now scopes the runtime directory at the
  root; a clean Git archive contains both Artifact directories, Linux backend
  quality passes, and the complete clean-room passed in
  `D:\Temp\User\reca-m0-acceptance-20260801-m1-final-fix-v2`.
